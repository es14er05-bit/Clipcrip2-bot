...
def detect_people(video):
    ...
    # Mehrere Stellen des Videos analysieren
    for i in range(10):
...
def create_ass(json_file, ass_file):
    ...
    # Manche Clips enthalten keine
    # brauchbaren Word-Timestamps.
    # Dann wird der Clip trotzdem verarbeitet.
    if not words:

        print(
            "Keine Word-Timestamps gefunden."
        )

        print(
            "Erstelle leere Untertiteldatei."
        )
...
def create_video(
    ...
    # Original Twitch clip is assumed
    # to be 1920x1080.
...
def main():
    ...
    print(
        f"{successful} von "
        f"{min(len(videos), 5)} "
        "Videos erfolgreich verarbeitet."
    )
