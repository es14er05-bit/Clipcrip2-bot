...
OUTPUT_DIR = ROHAT_ROOT / "tiktok_ready"
...
def build_session():
    ...
    session.headers.update({
        "User-Agent":
            "ClipCrip4-Rohat-GitHub-Actions/1.0",
...
def main():
    ...
    print(
        f"{len(files)} fertige "
        "ClipCrip4 Rohat-Videos gefunden."
    )

    if len(files) != 5:
        raise RuntimeError(
            "Es müssen genau 5 fertige "
            "Rohat-MP4-Dateien vorhanden sein. "
    ...
    for index, path in enumerate(
        ...
        print(
            f"CLIPCRIP4 ROHAT DRIVE VIDEO {index}/5"
        )
